...
2단계(.env 쓰기 + 게이트웨이 재시작)가 매 3분 크론 사이클마다 승인 대기
타임아웃으로 실패 → 다음 사이클이 처음부터 새 터널을 또 만드는 게
반복됐다(터널 주소가 계속 바뀐 게 그 증거). **크론(무인) 방식으로는 이
승인 단계를 넘을 수 없다** — 사람이 그 자리에서 실시간으로 승인해야
하는데 크론 세션은 사람이 없으면 그냥 끝난다.

**구솔라(신/구 어느 쪽이든)는 이 웹훅 전환 작업을 더 이상 자동으로
재시도하지 마라.** 신솔라 텔레그램은 이미 정상 작동하므로 급한 용도는
커버된다. 재개하려면 사장님이 인터랙티브 세션에 실시간으로 붙어서
승인까지 직접 진행해야 한다 — 그 전까지는 이 파일을 다시 pending으로
옮기지 않는다.
