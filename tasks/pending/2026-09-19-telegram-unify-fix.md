...
2. **확인 부탁형** (`요청:` 헤더가 있을 때): `탐 : 사장님, 확인 부탁드려요. <제목>. <요청 문장 그대로>`
   - 예(지금 가능한 행동만 요청): `탐 : 사장님, 확인 부탁드려요. 알림문구 다듬기가 끝났어요. 탐 세션에서 맘에 드는지 말씀해 주세요.`
   - 텔레그램 회신 경로가 생기면(백로그 21번) `맘에 드시면 '확인' 메세지 주세요.` 같은 요청을 쓸 수 있다. 사장님 확정 문구(원문)는 `탐 : 사장님, 확인 부탁드려요. 알림문구 다듬기가 끝났어요. 맘에 드시면 '확인' 메세지 주세요.`
